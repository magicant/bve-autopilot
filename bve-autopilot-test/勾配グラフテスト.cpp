...
namespace bveautopilottest
{
    ...
    public:
        ...
        TEST_METHOD(負の勾配一つのグラフの比エネルギー差)
        {
            ...
            Assert::AreEqual(
                0.0, g.比エネルギー差({12.5_m, 15.0_m}).value, 0.0);
            Assert::AreEqual(
                0.0, g.比エネルギー差({5.0_m, 12.5_m}).value, 0.0);
            Assert::AreEqual(
                0.0, g.比エネルギー差({15.0_m, 21.0_m}).value, 0.0);
        }
        ...
        TEST_METHOD(負の部分的勾配のグラフの比エネルギー差)
        {
            ...
            Assert::AreEqual(
                0.0, g.比エネルギー差({0.0_m, 30.0_m}).value, 0.0);
            Assert::AreEqual(
                0.0, g.比エネルギー差({0.0_m, 15.0_m}).value, 0.0);
            Assert::AreEqual(
                0.0, g.比エネルギー差({0.0_m, 40.0_m}).value, 0.0);
        }
        ...
        TEST_METHOD(負から正になる勾配のグラフの比エネルギー差)
        {
            ...
            Assert::AreEqual(
                0.0, g.比エネルギー差({0.0_m, 10.0_m}).value, 0.0);
            Assert::AreEqual(
                0.0, g.比エネルギー差({0.0_m, 20.0_m}).value, 0.0);
            Assert::AreEqual(
                0.0, g.比エネルギー差({0.0_m, 22.5_m}).value, 0.01);
            Assert::AreEqual(
                0.09, g.比エネルギー差({0.0_m, 25.0_m}).value, 0.01);
            Assert::AreEqual(
                0.37, g.比エネルギー差({0.0_m, 27.5_m}).value, 0.01);
            Assert::AreEqual(
                0.83, g.比エネルギー差({0.0_m, 30.0_m}).value, 0.01);
            Assert::AreEqual(
                1.93, g.比エネルギー差({0.0_m, 35.0_m}).value, 0.01);
        }
        ...
        TEST_METHOD(正から負になる勾配のグラフの比エネルギー差)
        {
            ...
            Assert::AreEqual(
                1.18, g.比エネルギー差({0.0_m, 23.0_m}).value, 0.01);
            Assert::AreEqual(
                1.18, g.比エネルギー差({0.0_m, 35.0_m}).value, 0.01);
        }
