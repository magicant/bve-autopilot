...
namespace autopilot
{
    ...
    struct 減速パターン
    {
        ...
        std::pair<速度型, 加速度型> 期待速度と期待減速度(米 現在位置)
            const;
        速度型 期待速度(米 現在位置) const {
            return 期待速度と期待減速度(現在位置).first;
        }

        加速度型 出力減速度(米 現在位置, 速度型 現在速度) const;
        int 出力制動ノッチ(
            米 現在位置, 速度型 現在速度, int 現在制動ノッチ,
            加速度型 勾配影響, const 共通状態 &状態) const;
        bool 力行する余裕あり(
            int 力行ノッチ, 加速度型 想定加速度, 秒 想定惰行時間,
            加速度型 勾配影響, const 共通状態 &状態) const;
