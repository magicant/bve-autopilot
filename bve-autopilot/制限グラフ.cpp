...
namespace autopilot
{
    ...
    速度型 制限グラフ::現在常用パターン速度(const 共通状態 &状態) const
    {
        ...
        加速度型 標準減速度 = 状態.制動().常用最大減速度();

        for (const 制限区間 &区間 : _区間リスト) {
            加速度型 勾配影響 = std::max(状態.進路勾配加速度(区間.始点), 0.0);
            加速度型 目標減速度 = 標準減速度 - 勾配影響;
            減速パターン パターン = 区間.対応パターン(目標減速度, 0, 0);
            速度 = std::min(速度, パターン.単純期待速度(状態.現在位置()));
        }
        return 速度;
    }

    int 制限グラフ::出力ノッチ(
        const 共通状態 & 状態, 時間型 時間マージン,
        距離型 停止位置マージン, 速度型 速度マージン) const
    {
