...
namespace autopilot
{
    ...
    /// 開始位置よりも手前側において列車が開始位置に近付いてゆくときに
    /// 減速するための出力減速度を計算します。
    /// 照査下パターン出力減速度よりも高い基準減速度に従います。
    mps2 orp::接近パターン出力減速度(
        const 運動状態 &運動状態, const 共通状態 &状態) const
    {
        減速パターン パターン{
            ...
            _開始照査速度,
            状態.目安減速度()};
        mps 期待速度 = パターン.期待速度(運動状態.位置, 状態.勾配());
        mps2 期待減速度 = パターン.期待減速度(運動状態.位置, 状態.勾配());
        mps2 出力減速度 =
            期待減速度 * (運動状態.速度 / 期待速度) +
            (運動状態.速度 - 期待速度) / 下接近時間;
        return 出力減速度;
    ...
    自動制動自然数ノッチ orp::出力制動ノッチ(
        const 運動状態 &運動状態, const 共通状態 &状態) const
    {
        mps2 平坦時減速度1 = 照査下パターン出力減速度(運動状態);
        mps2 平坦時減速度2 = 接近パターン出力減速度(運動状態, 状態);
        mps2 平坦時減速度3 = 下限照査出力減速度(運動状態);
        mps2 平坦時減速度 = std::max(
            std::min(平坦時減速度1, 平坦時減速度2),
            平坦時減速度3);
    ...
    std::pair<mps, 区間> orp::最低許容速度区間(区間 範囲) const
    {
        return {mps::無限大(), {範囲.終点, 範囲.終点}};
    }
