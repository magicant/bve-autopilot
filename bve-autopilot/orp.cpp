...
namespace autopilot
{
    ...
    orp::orp() :
        _信号指示{-1},
        _照査パターン{米::無限大(), 照査速度下限, 0, 0},
        _運転パターン{米::無限大(), 最終目標速度, 0, 0},
    ...
    void orp::リセット()
    {
        _照査パターン.目標位置 = _運転パターン.目標位置 = 米::無限大();
        _出力ノッチ = std::numeric_limits<int>::max();
    }

    void orp::設定(速度型 初期照査速度, 米 初期位置, 米 限界位置)
    {
        ...
        const 共通状態 &状態, const 信号順守 &信号)
    {
        ...
        case 12: { // ORP 動作開始 (メトロ総合プラグイン互換)
            ...
            米 残距離 = 地上子.Optional <= 48 ? 48.0_m : 79.0_m;
        ...
        case 1012: // 信号現示受信
                ...
                地上子.Distance > 0)
            {
                速度型 初速度 = 信号.現在制限速度(状態);
                米 開始位置 =
                    状態.現在位置() + static_cast<米>(地上子.Distance);
                米 orp距離 = 初速度 <= mps_from_kmph(30) ? 48.0_m : 79.0_m;
