...
namespace autopilot {
    ...
    tasc::tasc() :
        _目標停止位置のある範囲(未設定時の目標停止位置範囲),
        _調整した目標停止位置(m::無限大()),
        _直前目標停止位置受信位置(m::quiet_NaN()),
        _直前目標停止位置受信残距離(m::quiet_NaN()),
        _最大許容誤差(デフォルト最大許容誤差),
        _目標減速度(2.5_kmphps),
        _緩解(false),
        _出力ノッチ(緩解指令)
    ...
    void tasc::リセット()
    {
        _目標停止位置のある範囲.set(未設定時の目標停止位置範囲);
        _調整した目標停止位置 = m::無限大();
        _直前目標停止位置受信位置 = m::quiet_NaN();
        _最大許容誤差 = デフォルト最大許容誤差;
        _緩解 = false;
    ...
    void tasc::目標停止位置を更新(const 共通状態 &状態)
    {
        ...
        区間 受信した目標停止位置のある範囲{
            直前位置 + _直前目標停止位置受信残距離 - 0.0001_m,
            現在位置 + _直前目標停止位置受信残距離 + 0.0001_m};

        // これまでに分かっている範囲と組み合わせる
        区間 絞り込んだ範囲 = 重なり(
            受信した目標停止位置のある範囲, _目標停止位置のある範囲.get());
        ...
        if (!絞り込んだ範囲.含む(_目標停止位置のある範囲.get())) {
            _目標停止位置のある範囲.set(絞り込んだ範囲);
        }
