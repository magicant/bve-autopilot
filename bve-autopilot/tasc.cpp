...
namespace autopilot {
    ...
    void tasc::経過(const 共通状態 & 状態)
    {
        ...
        case 制御状態::制動:
            if (std::abs(現在速度) < 0.05) {
                ...
                モデル.指定時間走行(5);
                距離型 空走距離 = モデル.位置();
                int 予想出力 = 出力計算(残距離 - 空走距離, 現在速度, 状態);
    ...
    int tasc::出力計算_標準(距離型 残距離, 速度型 現在速度, const 共通状態 & 状態)
    {
        加速度型 目標減速度 = 状態.常用最大減速度() * 0.8;
        ...
        double 出力制動ノッチ = 状態.目標制動ノッチ(出力減速度);
        if (出力減速度 < 目標減速度) {
