...
namespace autopilot {
    ...
    void tasc::リセット()
    {
        ...
        _制御リセット条件 = デフォルト制御リセット条件;
        _緩解条件 = デフォルト緩解条件;
        _制御リセット時刻 = _緩解時刻 = static_cast<時刻>(s::無限大());
    }
    ...
    void tasc::手動ブレーキ条件ならイベント発動(const 共通状態 &状態)
    {
        if (!状態.戸閉()) {
            ...
            if (手動 >= 自動) {
                イベント発動(イベント::手動ブレーキ, 状態.現在時刻());
            }
        }
    }

    void tasc::イベント発動(イベント イベント, 時刻 現在時刻)
    {
        if (_制御リセット条件.タイミング == イベント) {
            _制御リセット時刻 =
                std::min(_制御リセット時刻, 現在時刻 + _制御リセット条件.遅延);
        }
        if (_緩解条件.タイミング == イベント) {
            _緩解時刻 = std::min(_緩解時刻, 現在時刻 + _緩解条件.遅延);
        }
    }
