...
namespace autopilot
{
        ...
        制限グラフ &追加先グラフ, 距離型 tasc目標停止位置) const
    {
        ...
        if (信号速度 == 0) {
            for (const auto &照査 : 停止信号前照査一覧) {
                距離型 照査位置 = 照査.first;
                速度型 照査速度 = 照査.second;
                制限グラフに制限区間を追加(追加先グラフ, 照査位置, 照査速度);
                if (照査速度 == 0) {
                    停止位置 = std::min(停止位置, 照査位置);
                }
            }

            if (atc用の信号指示である(信号指示)) {
                // ちょっと次の閉塞に入ったところで止める
                制限位置 += 1.0;
            }
            else if (制限位置 <= 停止位置) {
        ...
        制限グラフに制限区間を追加(追加先グラフ, 制限位置, 信号速度);
    }
    ...
    {
        _信号グラフ.消去();
        _現在閉塞.制限グラフに追加(_信号グラフ, _tasc目標停止位置);
        for (const auto &b : _前方閉塞一覧) {
            const 閉塞型 &閉塞 = b.second;
            閉塞.制限グラフに追加(_信号グラフ, _tasc目標停止位置);
        }
    }
