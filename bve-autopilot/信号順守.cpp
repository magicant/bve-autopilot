...
namespace autopilot
{
        ...
        制限グラフ &追加先グラフ, 距離型 始点_, 速度型 速度) const
    {
        ...
        追加先グラフ.制限区間追加(始点_, 速度);
    }

    void 信号順守::閉塞型::制限グラフに追加(制限グラフ &追加先グラフ) const
    {
        if (信号速度 == 0) {
            for (const auto &照査 : 停止信号前照査一覧) {
                制限グラフに制限区間を追加(
                    追加先グラフ, 照査.first, 照査.second);
            }
        }
        制限グラフに制限区間を追加(追加先グラフ, 始点, 信号速度);
    }
    ...
    void 信号順守::リセット()
    {
        ...
        if (!is_atc()) {
            ...
            // 現在閉塞の状態を維持する
            _現在閉塞 = 閉塞型{
                _現在閉塞.信号指示, _現在閉塞.信号速度, _現在閉塞.始点};
        }
    ...
    int 信号順守::出力ノッチ(
        const 共通状態 &状態, const tasc &tasc) const
    {
        距離型 停止マージン;
        時間型 時間マージン;
        if (is_atc()) {
