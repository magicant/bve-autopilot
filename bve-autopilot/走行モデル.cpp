...
namespace autopilot {
    ...
    加速度型 走行モデル::指定時間走行(
        秒 時間, 加速度型 初加速度, 加加速度型 加加速度)
    {
        // FIXME 単位を正しく扱う
        _位置 += static_cast<米>(時間.value * (_速度 + 時間.value * (初加速度 + 時間.value * 加加速度 / 3) / 2));
    ...
    void 走行モデル::指定距離走行(米 距離, 加速度型 加速度)
    {
        if (距離 == 0.0_m) {
            return;
        }
        if (加速度 == 0) {
            _時刻 += static_cast<秒>(距離.value / _速度); // FIXME 単位を正しく扱う
            _位置 += 距離;
            return;
        }
        ...
        _時刻 += static_cast<秒>((新速度 - _速度) / 加速度); // FIXME 単位を正しく扱う
        _速度 = 新速度;
        _位置 += 距離;
    }

    void 走行モデル::指定位置まで走行(米 位置, 加速度型 加速度)
    {
        指定距離走行(位置 - _位置, 加速度);
    ...
    void 走行モデル::指定速度まで走行(速度型 速度, 加速度型 加速度)
    {
        ...
        指定時間走行(static_cast<秒>((速度 - _速度) / 加速度), 加速度); // FIXME 単位を正しく扱う
        _速度 = 速度; // 誤差をなくすため直接再代入する
    }

    加速度型 走行モデル::指定位置まで走行(
        米 位置, 加速度型 初加速度, 加加速度型 加加速度)
    {
    ...
    {
        auto 時間 = static_cast<秒>((終加速度 - 初加速度) / 加加速度); // FIXME 単位を正しく扱う
        指定時間走行(時間, 初加速度, 加加速度);
    }
        ...
        速度型 速度, 加速度型 初加速度, 加加速度型 加加速度)
    {
        // 速度を位置、加速度を速度だと思って
        // 新しい速度とか速度をシミュレートする
        走行モデル 速度モデル{static_cast<米>(_速度), 初加速度, _時刻}; // FIXME 単位を正しく扱う
        速度モデル.指定位置まで走行(static_cast<米>(速度), 加加速度); // FIXME 単位を正しく扱う
