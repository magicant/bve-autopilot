...
namespace autopilot {
    ...
    {
        // FIXME 単位を正しく扱う
        _位置 += 時間 * (_速度 + static_cast<mps>(時間.value * (初加速度 + 時間.value * 加加速度 / 3) / 2));
        _速度 += static_cast<mps>(時間.value * (初加速度 + 時間.value * 加加速度 / 2));
        _時刻 += 時間;
    ...
    void 走行モデル::指定距離走行(m 距離, 加速度型 加速度)
    {
        ...
        mps 新速度 = static_cast<mps>(std::sqrt(_速度.value * _速度.value + 2 * 距離.value * 加速度)); // FIXME 単位を正しく扱う
        if (!(新速度 >= 0.0_mps)) {
            新速度 = 0.0_mps;
        }
        _時刻 += static_cast<s>((新速度 - _速度).value / 加速度); // FIXME 単位を正しく扱う
        _速度 = 新速度;
        _位置 += 距離;
    }
    ...
    void 走行モデル::指定速度まで走行(mps 速度, 加速度型 加速度)
    {
        if (速度 == _速度) {
            return;
        }
        指定時間走行(static_cast<s>((速度 - _速度).value / 加速度), 加速度); // FIXME 単位を正しく扱う
        ...
        mps 速度, 加速度型 初加速度, 加加速度型 加加速度)
    {
        ...
        走行モデル 速度モデル{
            ...
            static_cast<mps>(初加速度), // FIXME 単位を正しく扱う
            _時刻};
        速度モデル.指定位置まで走行(static_cast<m>(速度.value), 加加速度); // FIXME 単位を正しく扱う
        指定時刻まで走行(速度モデル.時刻(), 初加速度, 加加速度);
        _速度 = 速度; // 誤差をなくすため直接再代入する
        return 速度モデル.速度().value; // FIXME 単位を正しく扱う
    }

    加速度型 走行モデル::距離と速度による加速度(
        m 距離, mps 初速度, mps 終速度)
    {
        return (終速度.value * 終速度.value - 初速度.value * 初速度.value) / 距離.value / 2.0; // FIXME 単位を正しく扱う
    }
