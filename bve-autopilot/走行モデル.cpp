...
namespace autopilot {
    ...
    mps2 走行モデル::指定時間走行(s 時間, mps2 初加速度, 加加速度型 加加速度)
    {
        // FIXME 単位を正しく扱う
        _位置 += 時間 * (_速度 + 時間 * (初加速度 + static_cast<mps2>(時間.value * 加加速度) / 3.0) / 2.0);
        _速度 += 時間 * (初加速度 + static_cast<mps2>(時間.value * 加加速度) / 2.0);
        _時刻 += 時間;
        return 初加速度 + static_cast<mps2>(時間.value * 加加速度);
    }

    mps2 走行モデル::指定時刻まで走行(
        s 時刻, mps2 初加速度, 加加速度型 加加速度)
    {
    ...
    void 走行モデル::等加加速度で指定加速度まで走行(
        mps2 初加速度, mps2 終加速度, 加加速度型 加加速度)
    {
        auto 時間 = static_cast<s>((終加速度 - 初加速度).value / 加加速度); // FIXME 単位を正しく扱う
        指定時間走行(時間, 初加速度, 加加速度);
    }

    mps2 走行モデル::指定速度まで走行(
        mps 速度, mps2 初加速度, 加加速度型 加加速度)
    {
        // 速度を位置、加速度を速度だと思って
        // 新しい速度とか速度をシミュレートする
        走行モデル 速度モデル{
