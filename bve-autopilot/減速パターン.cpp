...
namespace autopilot
{
    ...
    std::pair<mps, mps2> 減速パターン::延長期待状態(m 現在位置) const
    {
        // 目標位置・目標速度の状態から逆算する
        走行モデル 走行(目標位置, 目標速度);
    ...
    std::pair<mps, mps2> 減速パターン::期待状態(m 現在位置) const
    {
        if (現在位置 >= 目標位置) {
    ...
    mps2 減速パターン::減速用出力減速度(m 現在位置, mps 現在速度) const
    {
        if (現在位置 >= 目標位置) {
            return mps2::無限大();
        }

        auto [期待速度, 期待減速度] = 延長期待状態(現在位置);
    ...
    {
        const 制動特性 &制動 = 状態.制動();
        mps2 出力減速度1 = 減速用出力減速度(現在位置, 現在速度);
