...
namespace autopilot
{
        ...
        m 現在位置, const 勾配グラフ &勾配) const
    {
        ...
        // 厳密には勾配加速度の計算位置が違うが、大体同じなので
        mps2 遷移勾配加速度 = std::max(勾配.列車勾配加速度(目標位置), 0.0_mps2);
        mps2 遷移減速度 = 初期減速度 - 遷移勾配加速度;
        ...
        m2ps2 勾配比エネ差 = 勾配.下り勾配比エネルギー({現在位置, 遷移位置});
        mps 期待速度 = 加算(走行.速度(), -勾配比エネ差);
        mps2 現在勾配加速度 = std::max(勾配.列車勾配加速度(現在位置), 0.0_mps2);
        mps2 期待減速度 = 初期減速度 - 現在勾配加速度;
        return {期待速度, 期待減速度};
    }
