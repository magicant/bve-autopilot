...
namespace autopilot
{
    ...
    std::pair<mps, 加速度型> 減速パターン::期待速度と期待減速度(
        m 現在位置) const
    {
        if (現在位置 >= 目標位置) {
            return { 目標速度, 0 };
    ...
    加速度型 減速パターン::出力減速度(m 現在位置, mps 現在速度) const
    {
        if (現在位置 >= 目標位置) {
            if (目標速度 > 0.0_mps) {
                return (現在速度 - 目標速度).value / 2; // FIXME 単位を正しく扱う
            }
            else {
                return std::numeric_limits<加速度型>::infinity();
            }
        }
        ...
        加速度型 出力減速度1 =
            期待減速度 * (現在速度 / 期待速度) - (期待速度 - 現在速度).value / 2; // FIXME 単位を正しく扱う
    ...
    {
        const 制動特性 &制動 = 状態.制動();
        加速度型 出力減速度 = this->出力減速度(現在位置, 現在速度);
        double 出力制動ノッチ相当 = 制動.自動ノッチ(出力減速度 + 勾配影響);

        if (目標位置 >= 現在位置) {
            // 小刻みに制動ノッチを変化させるのを防ぐ
            double 補正ノッチ = 0.25 * std::min(1.0, 現在速度.value / (5 * 初期減速度)); // FIXME 単位を正しく扱う
    ...
    int 減速パターン::出力ノッチ(const 共通状態 &状態) const
    {
            ...
            出力制動ノッチ < 現在制動ノッチ)
        {
            ...
            s 猶予 = static_cast<s>( // FIXME 単位を正しく扱う
                出力減速度 > 0 ?
                std::clamp((現在速度 - 目標速度).value / 4 / 出力減速度, 1.0, 3.0) :
                1.0);
            走行モデル モデル = 状態.現在走行状態();
            モデル.指定時間走行(猶予, -出力減速度);
        ...
        if (力行する余裕あり(
            状態.車両仕様().PowerNotches, mps_from_kmph(5.0), 5.0_s,
            想定勾配影響, 状態))
        {
        ...
        if (力行する余裕あり(1, mps_from_kmph(2.5), 1.0_s, 想定勾配影響, 状態))
        {
            return 1;
        }
        return 0;
