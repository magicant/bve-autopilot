...
namespace autopilot
{
    ...
    mps 減速パターン::延長期待速度(m 現在位置, const 勾配グラフ &勾配) const
    {
        m2ps2 目標位置比エネ = 0.5 * 目標速度 * 目標速度;
        m2ps2 平坦行路比エネ = (目標位置 - 現在位置) * 目標減速度;
        m2ps2 勾配比エネ差 = 勾配.下り勾配比エネルギー({現在位置, 目標位置});
        m2ps2 補正行路比エネ = 平坦行路比エネ - 勾配比エネ差;
        m2ps2 現在位置比エネ = 目標位置比エネ + 補正行路比エネ;
        mps 期待速度 = sqrt(2.0 * 現在位置比エネ);
        return 期待速度;
    }

    mps2 減速パターン::延長期待減速度(m 現在位置, const 勾配グラフ &勾配) const
    {
        mps2 勾配加速度 = 勾配.列車勾配加速度(現在位置);
        mps2 補正加速度 = std::max(勾配加速度, 0.0_mps2);
        mps2 期待減速度 = 目標減速度 - 補正加速度;
        return 期待減速度;
    }
        ...
        m 現在位置, mps 現在速度, const 勾配グラフ &勾配) const
    {
        ...
        mps 期待速度 = 延長期待速度(現在位置, 勾配);
        mps2 期待減速度 = 延長期待減速度(現在位置, 勾配);
    ...
    mps2 減速パターン::収束用出力減速度(mps 現在速度) const
    {
        if (目標速度 == 0.0_mps) {
            return mps2::無限大();
        }
    ...
    {
        const 制動特性 &制動 = 状態.制動();
        mps2 出力減速度1 = 減速用出力減速度(現在位置, 現在速度, 状態.勾配());
        mps2 出力減速度2 = 収束用出力減速度(現在速度);
        自動制動実数ノッチ 出力実数1 = 制動.自動ノッチ(出力減速度1);
        自動制動実数ノッチ 出力実数2 = 制動.自動ノッチ(出力減速度2);

        // 小刻みに制動ノッチを変化させるのを防ぐ
        double 補正ノッチ =
            0.25 * std::min(1.0, 現在速度 / (5.0_s * 目標減速度));
        if (出力実数1 >= 自動制動実数ノッチ{現在制動ノッチ}) {
            出力実数1.value -= 補正ノッチ;
        }
        else {
            出力実数1.value += 補正ノッチ;
        ...
        自動制動自然数ノッチ 出力自然数2 = ノッチ切り上げ(出力実数2);
