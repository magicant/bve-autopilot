...
namespace autopilot
{
        ...
        mps2 勾配影響, const 共通状態 &状態) const
    {
        ...
        if (目標位置 >= 現在位置) {
            ...
            if (出力実数 >=
                自動制動実数ノッチ{static_cast<double>(現在制動ノッチ)})
            {
                出力実数.value -= 補正ノッチ;
            }
            else {
        ...
        if (出力実数.value > std::numeric_limits<int>::max()) {
            return static_cast<int>(制動.自動最大ノッチ().value);
        }
        int 出力制動ノッチ = static_cast<int>(出力実数.value);
        return std::clamp(
            出力制動ノッチ,
            0,
            static_cast<int>(制動.自動最大ノッチ().value));
    }
        ...
        mps2 勾配影響, const 共通状態 &状態) const
    {
        ...
        短く力行(
            モデル,
            autopilot::力行ノッチ{static_cast<unsigned>(力行ノッチ)},
            想定加速度, 状態);
    ...
    int 減速パターン::出力ノッチ(const 共通状態 &状態) const
    {
        ...
        int 出力制動ノッチ = this->出力制動ノッチ(
            状態.現在位置(), 現在速度, 現在制動ノッチ.value, 勾配影響, 状態);

        // 制動を弱めてもまたすぐ強くすることになるなら弱めない
        if (現在速度 >= static_cast<mps>(10.0_kmph) &&
            出力制動ノッチ < 現在制動ノッチ.value)
        {
            mps2 出力減速度 =
                状態.制動().減速度(
                    自動制動自然数ノッチ{static_cast<unsigned>(出力制動ノッチ)}) -
                勾配影響;
            s 猶予 = 出力減速度 > 0.0_mps2 ?
                std::clamp(
        ...
        if (目標速度 == 0.0_mps) {
            ...
            int 緩め制動ノッチ =
                static_cast<int>(std::ceil(緩め制動ノッチ実数.value));
