...
// 速度が高くなるほど惰行時間を延ばすことで、無駄な力行を抑制する。
        ...
        {
            // 惰行した間の勾配による速度変化を考慮する
            m2ps2 勾配比エネ差 =
                _状態.勾配().比エネルギー差({元の位置, 予想状態.位置()});
            運動状態 修正予想状態{
                予想状態.位置(),
                加算(予想状態.速度(), 勾配比エネ差),
                予想状態.時刻()};
            ...
            if (出力制動ノッチ(修正予想状態) != 自動制動自然数ノッチ{0}) {
                return false;
            }
        }

        // FIXME: 位置が元の位置に近すぎる時、力行後ほとんど惰行せずに制動がかかる

        // 惰行した間の勾配による速度変化によって速度が最大になるところを求める
        auto [位置, 勾配比エネ差] =
            _状態.勾配().最大比エネルギー差({元の位置, 予想状態.位置()});
        運動状態 修正予想状態{
            位置, 加算(予想状態.速度(), 勾配比エネ差), 予想状態.時刻()}; // ※
// ※ 時刻が正しくないが、どのみち出力制動ノッチの計算には関係ない

        // その時点でブレーキをかける必要があるなら力行しない
        return 出力制動ノッチ(修正予想状態) == 自動制動自然数ノッチ{0};
    }
