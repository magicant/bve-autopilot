...
namespace autopilot
{
    ...
    void ato::経過(const 共通状態 &状態)
    {
        ...
        if (_制御状態 == 制御状態::停止) {
            ...
            _出力ノッチ = std::min({
                自動制御指令{状態.最大力行ノッチ()},
                _制限速度1006.出力ノッチ(true, 状態),
                _制限速度1007.出力ノッチ(true, 状態),
                _制限速度6.出力ノッチ(true, 状態),
                _制限速度8.出力ノッチ(true, 状態),
                _制限速度9.出力ノッチ(true, 状態),
                _制限速度10.出力ノッチ(true, 状態),
                _orp.出力ノッチ(),
                _早着防止.出力ノッチ(),
                _急動作抑制.出力ノッチ(),
                });
    ...
    {
        mps 速度 = std::min({
            _制限速度1006.現在常用パターン速度(true, 状態),
            _制限速度1007.現在常用パターン速度(true, 状態),
            _制限速度6.現在常用パターン速度(true, 状態),
            _制限速度8.現在常用パターン速度(true, 状態),
            _制限速度9.現在常用パターン速度(true, 状態),
            _制限速度10.現在常用パターン速度(true, 状態),
            _信号.現在常用パターン速度(状態),
            });
