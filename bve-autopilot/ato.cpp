...
namespace autopilot
{
    ...
    void ato::経過(const 共通状態 &状態, const tasc &tasc状態)
    {
        ...
        if (_制御状態 == 制御状態::停止) {
            ...
            自動制御指令 信号ノッチ = _信号.出力ノッチ(状態);
            自動制御指令 総合ノッチ = std::min({
                制限速度ノッチ, 信号ノッチ, _早着防止.出力ノッチ() });

            _急動作抑制.経過(総合ノッチ, 状態);
            _出力ノッチ = _急動作抑制.出力ノッチ();
            _出力ノッチ = std::min(_出力ノッチ, 制限速度ノッチ);
            if (!_信号.is_atc()) {
                _出力ノッチ = std::min(_出力ノッチ, 信号ノッチ);
            }
        }
    ...
    void ato::レバー操作(const 共通状態 &状態) noexcept
    {
        if (状態.設定().ato一時停止あり()) {
            _制御状態 = 制御状態::一時停止;
        }
