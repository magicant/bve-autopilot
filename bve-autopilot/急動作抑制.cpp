...
namespace autopilot
{
    ...
    mps2 急動作抑制::新出力減速度(
        int 入力ノッチ, const 共通状態 &状態, bool is_atc) const
    {
        // 力行をやめた直後は制動しない
    ...
    int 急動作抑制::新出力ノッチ(int 入力ノッチ, const 共通状態 &状態) const
    {
        if (状態.停車中()) {
            return 入力ノッチ;
        }
        ...
        if (新出力ノッチ < static_cast<double>(入力ノッチ)) {
            新出力ノッチ = std::ceil(新出力ノッチ);
        }
        else {
            新出力ノッチ = std::floor(新出力ノッチ);
        }
        return static_cast<int>(新出力ノッチ);
    }
