...
namespace autopilot
{
        ...
        自動制御指令 入力ノッチ, const 共通状態 &状態) const
    {
        ...
        自動制動実数ノッチ 新出力ノッチ = 状態.制動().自動ノッチ(_出力減速度);
        if (新出力ノッチ < 自動制動実数ノッチ{入力ノッチ.制動成分()}) {
            新出力ノッチ.value = std::ceil(新出力ノッチ.value);
        }
        else {
            新出力ノッチ.value = std::floor(新出力ノッチ.value);
        }
