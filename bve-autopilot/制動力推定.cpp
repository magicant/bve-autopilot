...
namespace autopilot
{
    ...
    void 制動力推定::経過(double 前回出力割合, const 共通状態 &状態)
    {
    ...
    void 制動力推定::最大減速度を推定(double 割合, const 共通状態 &状態)
    {
        秒 フレーム = 状態.現在時刻() - _前回観測時刻;
        if (フレーム <= 秒(0.0) || 秒(1.0) <= フレーム) {
            return;
        }
