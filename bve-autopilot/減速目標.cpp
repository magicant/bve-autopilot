...
namespace autopilot
{
    ...
    減速パターン 減速目標::主パターン(mps 現在速度, const 勾配グラフ &勾配)
        const
    {
        if (_速度 <= 0.0_mps) {
        ...
        // 現在速度が目標速度に近いときは接点を低くする。
        // そうしないと、接点に到達する前に目標速度まで減速する動きになり
        // 結果として減速タイミングが無駄に早くなってしまう。
        if (接点速度差 > 0.5 * (現在速度 - _速度)) {
            接点速度差 = 0.5 * (現在速度 - _速度);
            接点減速度 = std::max(接点速度差 / 接近時間, 接点減速度下限);
            接点速度差 = 接点減速度 * 接近時間;
        }
