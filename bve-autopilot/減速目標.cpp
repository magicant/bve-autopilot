...
namespace autopilot
{
    ...
    namespace
    {
        ...
        減速パターン 二点間パターン(
            m 復帰地点, mps 復帰速度, m 現在位置, mps 現在速度,
            const 勾配グラフ &勾配)
        {
            m 距離 = 復帰地点 - 現在位置;
            m2ps2 復帰地点比エネ2 = 復帰速度 * 復帰速度;
            m2ps2 現在位置比エネ2 = 現在速度 * 現在速度;
            m2ps2 速度比エネ差 = 0.5 * (現在位置比エネ2 - 復帰地点比エネ2);
            m2ps2 勾配比エネ差 = 0.0_m2ps2;
            // TODO 勾配.下り勾配比エネルギー({現在位置, 復帰地点});
            mps2 基準減速度 = (速度比エネ差 + 勾配比エネ差) / 距離;
            return 減速パターン{復帰地点, 復帰速度, 基準減速度};
        }
        ...
        const 走行モデル &運動状態, const 勾配グラフ &勾配) const
    {
        ...
        mps2 出力減速度2 = パターン2.期待減速度(運動状態.位置(), 勾配);
        ...
        const 走行モデル &運動状態, const 共通状態 &状態) const
    {
        ...
        mps2 勾配加速度 = {}; // TODO 状態.勾配().列車勾配加速度(現在位置);
    ...
    自動制動自然数ノッチ 減速目標::出力制動強めノッチ(const 共通状態 &状態)
        const
    {
        // 現在状態での制動ノッチを求める
        走行モデル 運動状態 = 状態.現在走行状態();
        自動制動自然数ノッチ 制動ノッチ = 出力制動ノッチ(運動状態, 状態);

        if (状態.現在速度() < static_cast<mps>(10.0_kmph)) {
            return 制動ノッチ;
        }
        ...
        mps2 減速度 = 状態.制動().減速度(制動ノッチ);
        // TODO -状態.勾配().勾配加速度(運動状態.位置());
        s 猶予 = 減速度 > 0.0_mps2 ?
            std::clamp(
                (運動状態.速度() - _速度) / 4.0 / 減速度, 1.0_s, 3.0_s) :
            運動状態.位置() < _位置 ? 1.0_s : 3.0_s;
        運動状態.指定時間走行(猶予, -減速度);
        自動制動自然数ノッチ 制動ノッチ2 = 出力制動ノッチ(運動状態, 状態);
        if (制動ノッチ2 <= 制動ノッチ) {
            return 制動ノッチ;
        }
        ...
        const 共通状態 &状態) const
    {
        ...
        短く力行(運動状態, ノッチ, 想定加速度, 状態);

        // その後、惰行して……
        運動状態.指定時間走行(想定惰行時間);

        // TODO 惰行した間の下り勾配による加速を考慮する
    ...
    力行ノッチ 減速目標::出力力行ノッチ(const 共通状態 &状態) const
    {
        // 制限速度まで余裕があるなら全力で力行する
        if (力行する余裕あり(状態.最大力行ノッチ(), 5.0_kmphps, 5.0_s, 状態)) {
            return 状態.最大力行ノッチ();
        }
        ...
        // 止まりかけならとにかく前に進む
        if (状態.現在速度() < static_cast<mps>(0.5_kmph)) {
            return 力行ノッチ{1};
        }

        // 余裕があれば弱く力行
        if (力行する余裕あり(力行ノッチ{1}, 2.5_kmphps, 1.0_s, 状態)) {
            return 力行ノッチ{1};
        }
    ...
    自動制御指令 減速目標::出力ノッチ(const 共通状態 &状態) const
    {
        自動制動自然数ノッチ 制動ノッチ = 出力制動強めノッチ(状態);

        // 停止直前はノッチを緩めて衝撃を抑える
        if (_速度 == 0.0_mps) {
            自動制動自然数ノッチ 緩めノッチ = 緩め制動ノッチ(状態);
